...
int main()
{
	...
	std::vector<float> lenan = 	addGaussianNoise(lena8);

	std::vector<float> blurred  = blur(lena8, kernel);

	std::vector<float> nblur = blur(lenan, kernel);
	store("nblur.raw", nblur);
	
	std::vector<float> bnoise =  addGaussianNoise(blur(lena8, kernel));
	store("bnoise.raw", bnoise);

	printf("PSNR bnoise: %.5f dB\n", psnr(lena8, bnoise, 1.0));


	printf("PSNR nblur: %.5f dB\n", psnr(lena8, nblur, 1.0));
	return 0;
}